...
class ESN(object):
    ...
    def initialize(self): 
        np.random.seed(42)
        self.Win=np.random.uniform(-0.3,0.3, size=(self.res_size,1+self.in_size))
        self.Win[:,0]=np.random.uniform(-0.8,0.8, size=(self.res_size,))
...
def test_all(directory,folder,spectral_radius, n_range, nrmse, noise, euler=True, save=False):
    ...
    for file in csv_files:
        X,Y,nrmse_by_n,mi_by_n,filename= test(directory,file,folder,spectral_radius, n_range, nrmse, noise, euler, save, single=False)
...
def test(directory,file,folder,spectral_radius, n_range, nrmse, noise, euler=True, save=False, single=True):
    #init
    print(file)
    filename=file[file.index("list")+5:file.index(".csv")]
    ...
    for n in n_range:
        ...
        if single:
            ...
            if nrmse:
                ...
                print("Plot mi")
                plot_dict_i(filename, mi_by_n[n],MImax_n,n,nrmse=False)
    ...
    return X,Y_n,nrmse_by_n,mi_by_n,filename,MImax_n


# In[14]:


def plot_dict_i(key,dict_i,MImax_n,n,nrmse=True, single=True):
    x=[]
    y=[]
...
#Nrmse
X,Y,nrmse_i,mi_i,filename,MImax_n=test("Dataset1",file, "noise", 0.95, n_range, nrmse=True, noise=True)


# In[32]:


X,Y,nrmse_i,mi_i,filename, MImax_n=test("Dataset1",file, "noise", 1, n_range, nrmse= False, noise=True)
...
#Mi
X,Y,nrmse_i,mi_i,filename, MImax_n=test("Dataset1",file, "noise", 0.95, n_range, nrmse= False, noise=True)
exponential_fitting(MImax_n, MI=True)
